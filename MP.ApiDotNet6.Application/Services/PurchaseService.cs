...
namespace MP.ApiDotNet6.Application.Services
{
    public class PurchaseService : IPurchaseService
    {
        ...
        public async Task<ResultService<ICollection<PuchateDetailDTO>>> GetAsync()
        {
            var purchases = await _purchaseRepository.GetPurchasesAsync();
            return ResultService.Ok(_mapper.Map<ICollection<PuchateDetailDTO>>(purchases));
        }

        public async Task<ResultService<PuchateDetailDTO>> GetByIdAsync(int id)
        {
            var purchase = await _purchaseRepository.GetByIdAsync(id);
           if (purchase == null)
               return ResultService.Fail<PuchateDetailDTO>("Compra não encontrada!");
               return ResultService.Ok(_mapper.Map<PuchateDetailDTO>(purchase));
        }
